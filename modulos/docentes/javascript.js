...
function carregarNumeroPaginas() {
    ...
    esconderPaginacao()
    request = new Request()
    request.add("quantidadeDeRegistrosDocentes", "")
...
function carregarPagina(p) {
    if (carregando == false) {
        ...
        request = new Request()
        request.add("pegarTodosDocentes", "")
        ...
        request.send("GET", ["JSON"], (resposta) => {
            Object.keys(resposta).forEach(function (nome) {
                ...
                var disci = resposta[nome]["disci"];
                var tipo = resposta[nome]["tipo"];
                renderizarLinha(id, nome, disci, tipo)
            });
            mostrarTabela()
            mostrarPaginacao()
            carregando = false;
...
function renderizarLinha(id, nome, disci, tipo) {
    disci = disciplinaIDtoNome(disci.split("-")).join(", ")
    a =
        "<tr class=\"hover\">\
        <td>\
